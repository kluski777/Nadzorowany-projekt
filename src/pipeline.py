...
class InferencePipeline:
    ...
    def _load_models(self) -> None:
        ...
        self.autoencoder = ResidualConvtAutoEncoder.load_from_checkpoint(
            self.AUTOENCODER_PATH, map_location=self.device
        )
    ...
    def load_inpainter(self, cluster_id: int | None) -> tuple[ConvLatentInpainter, bool]:
        path = (
            Path(self.INPAINTER_DIR) / self.INPAINTER_PATTERN.format(cluster_id=cluster_id)
            if cluster_id is not None
            else Path(self.COMMON_INPAINTER_PATH)
        )
        ...
        if not path.exists():
            ...
            if not path.exists():
                raise FileNotFoundError("No inpainter found.")
    ...
    def postprocess(self, tensor: torch.Tensor) -> Image.Image:
        ...
        array = superresoluted.cpu().permute(1, 2, 0).numpy()
